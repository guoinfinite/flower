...
import com.example.flowers.service.AlarmService;
import com.example.flowers.ui.bean.Quotes;
import com.example.flowers.ui.bean.RecordDate;
...
public class MyDataHelper {
    ...
    private void addDateData(Context context) throws ParseException {
        ...
        if (cursor.getCount() == 0) {//此时应为第一次进入程序时才会调用该方法
            ...
            //TODO:解锁花：6
            SaveKeyValues.putIntValues("login_days", 12);//默认为一
            //定时服务
            ...
            if (TextUtils.equals(nowDate.substring(0, 10), lastTime.substring(0, 10))) {//日期为同一天
                ...
                if (days == 2) {
                    //获取之前的值
                    int loginValues = SaveKeyValues.getIntValues("login_days", 0) + 12;
                    loginValues += 1;
                    //存入新的值-->此时是连续登陆
                ...
                if (days != 1 && days != 2) {//不是同一天也不是隔一天
                    ...
                    SaveKeyValues.putIntValues("login_days", 12);//清空连续登陆的数据
                }
            }
        }
